...
class HoverAdicionarTarefa:
    ...
    def card_edit(self):
        ...
        CardManager.set_current_card(self.obter_card_visivel())

    def clear_fields(self):
        if self.edit:
            self.edit.content.limpar_campos()
        else:
            self.obter_card_visivel().limpar_campos()
    ...
    def obter_card_visivel(self):
        """Retorna o cartão visível."""
        for card in self.card_container:
            if card.visible:
                return card
        return self.card_container[0]
